...
BirdSound-Recognition 是一个面向 BirdCLEF2026 声景数据的鸟类声音识别课程项目。项目包含传统机器学习与深度学习对照实验、指标评估、报告材料，以及一个基于 FastAPI 的本地网页演示系统。

网页演示支持上传音频，自动生成波形图、Log-Mel 频谱图和 MFCC 特征图，并返回当前模型训练标签范围内的鸟类预测候选。预测结果会显示中文参考名、英文常用名、学名、标签编号、类群和置信度。

## 项目特点

- 使用 BirdCLEF2026 声景数据组织多标签识别实验。
- 包含传统模型对比：ExtraTrees、Linear SVM、KNN、Logistic Regression。
- 包含深度模型对比：CNN baseline 与 Attention-BiGRU。
- 支持 LRAP、Micro F1、Macro F1、Top-3 Hit Rate、Hamming Loss 等指标。
- 提供 FastAPI + 原生 HTML/JavaScript 网页演示界面。
- 网页结果优先展示鸟类候选，并用中文名和置信度方便演示。
- 保留课程报告、PPT、大纲、实验表格、可视化图表和网页截图。
...
.
├── 代码/
│   ├── scripts/                  # 实验运行、报告图表生成、网页自检脚本
│   └── src/
│       ├── birdclef_lsw/          # 核心实验、指标、模型和推理代码
│       ├── birdclef_lsw_web/      # FastAPI 网页演示入口
│       └── birdclef_step4/        # 本地演示兼容模块
├── 数据集/
│   └── 关键统计/                  # 报告和答辩所需的小型统计文件
├── 报告/
│   ├── 实验产物/                  # 模型、实验 CSV、网页截图、可视化图表
...
如果只运行网页演示，CPU 环境即可。重新训练深度学习模型时，建议使用 NVIDIA GPU 和完整 BirdCLEF2026 原始数据。
...
python -m uvicorn birdclef_lsw_web.app:app --host 127.0.0.1 --port 8000
```
...
网页启动后，可以上传 `.wav`、`.mp3`、`.ogg`、`.flac` 等音频文件进行识别。演示样例音频默认放在项目上一级目录的 `web_test_audio/` 中；也可以直接在网页里上传本地音频。

服务停止方式是在终端按 `Ctrl + C`。

## 网页识别结果说明

网页会优先显示鸟类预测结果，结果按照模型置信度从高到低排序。每个候选包含：

- 中文参考名和英文常用名，例如：`紫蓝鸦 (Purplish Jay)`
- 学名，例如：`Cyanocorax cyanomelas`
- 标签编号，例如：`purjay1`
- 类群，例如：`鸟类 / Aves`
- 置信度，例如：`置信度 57.4%`

注意：这里的置信度是模型输出的排序分数，不等同于严格的准确率。如果前几个候选分数很接近，说明模型对该音频的区分度不高，结果更适合作为候选参考。

当前演示模型只覆盖训练过的 75 个活动标签，并不能识别所有鸟类物种。如果上传音频对应的鸟类不在训练标签中，系统会返回训练标签范围内最接近的候选。
...
完整训练依赖原始 BirdCLEF2026 音频和上游 Step2/Step4 数据产物。仓库中主要保留展示、报告和已生成实验产物，不包含完整原始音频数据。

## 数据说明

`数据集/关键统计/` 中保留了报告和答辩所需的小型统计文件。完整 BirdCLEF2026 原始音频体积较大，不适合直接提交到 GitHub。

如需连接远程数据服务器，可在本地配置以下环境变量：
